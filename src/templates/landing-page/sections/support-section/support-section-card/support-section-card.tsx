...
const Root: FC<SupportSectionCardProps> = ({ children, ...props }) => (
  <div className={`flex flex-col text-left gap-2 rounded-lg p-6 md:p-12 ${props.className}`}>
    {children}
  </div>
);
...
const Title: FC<SupportSectionCardProps> = ({ children, ...props }) => (
  <strong className={`text-heading-sm text-gray-100 ${props.className}`}>
    {children}
  </strong>
);

const Text: FC<SupportSectionCardProps> = ({ children, ...props }) => (
  <p className={`text-body-sm text-gray-200 ${props.className}`}>
    {children}
  </p>
);
